...
// NEW: Structured flash (galing sa change_password.php at iba pa)
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('edit-profile-btn').onclick = function(e) {
        e.stopPropagation();
    ...
    document.getElementById('change-password-btn').onclick = function(e) {
        e.stopPropagation();
...
// Logout button SweetAlert confirmation + AJAX
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadNotifications() {
    fetch('client_get_notifications.php')
      .then(r => r.json())
      .then(data => {
        ...
        notifList.innerHTML = data.map(notif => `
          <div class="notif-item ${notif.is_read == 0 ? 'notif-unread' : ''}"
               ...
               data-link="${notif.link || ''}" style="cursor:pointer;">
            <div class="notif-main">${notif.message}</div>
  ...
  notifList.addEventListener('click', (e) => {
    ...
    .then(r => r.json())
    .then(() => {
      // Refresh UI before redirect for feedback
      loadNotifications();
      setTimeout(() => { window.location = link; }, 200);
    });
  });

  // Open/close dropdown (same as admin)
  ...
  // Realtime poll every 5s (same cadence as admin)
  loadNotifications();
