...
$stmt = $conn->prepare("SELECT payment_id, amount, status, payment_date FROM payment WHERE booking_id=? ORDER BY payment_id DESC LIMIT 1");
$stmt->bind_param('i', $booking_id);
$stmt->execute();
$last = $stmt->get_result()->fetch_assoc();
$stmt->close();

$stmt = $conn->prepare("SELECT COALESCE(SUM(amount),0) AS total_paid FROM payment WHERE booking_id=? AND status IN ('paid','cleared')");
$stmt->bind_param('i', $booking_id);
$stmt->execute();
...
:root{
  ...
  --bg:#f3f8fc; --surface:#fff; --border:#e8eef6; --pill:#2e5f91;
}
*{box-sizing:border-box}
html,body{margin:0;background:#ddd;}
.page{
  max-width:820px; margin:0 auto; background:#333; /* gray frame like screenshot */
}
.paper{
  background:#fff; margin:0 26px; /* leave dark border sides */
}
.header{
  padding:26px 26px 10px; position:relative; text-align:center;
}
.brand-mark{
  width:72px; height:72px; border-radius:50%; object-fit:cover; display:block; margin:0 auto 6px;
}
.h-top-info{
  position:absolute; right:26px; top:24px; text-align:right; color:#222; font-size:14px;
}
...
/* meta grid (left labels) */
.meta{
  padding:0 26px 10px;
  display:grid; grid-template-columns: 280px 1fr; gap:30px;
}
.m-block h5{
  margin:0 0 4px; color:#203b60; font-weight:800; font-size:14px; letter-spacing:.3px;
}
.m-block .val{
  background:#f3f8fd; border-radius:4px; padding:6px 10px; color:#2f4b66; font-size:14px;
...
/* section title */
.section-title{
  padding:16px 26px 8px; color:#2b4766; font-weight:800; font-size:14px;
}

/* table */
.table-wrap{ padding:0 26px 14px; }
.table{
  width:100%; border-collapse:collapse; font-size:14px; background:#fbfdff; border-radius:6px; overflow:hidden;
}
...
/* total pill to the right */
.total-pill-row{ padding:14px 26px 4px; display:flex; justify-content:flex-end; }
.total-pill{
  background:var(--pill); color:#fff; border-radius:6px; padding:10px 14px; display:inline-flex; align-items:center; gap:14px; min-width:220px; justify-content:space-between; font-weight:700;
}

/* signature */
.footer{ padding:18px 26px 26px; }
.sig{ margin-top:28px; }
.sig .line{ border-top:2px solid #2a3d5e; width:260px; }
.sig .who{ margin-top:6px; color:#2a3d5e; font-weight:700; }

/* print button (only on screen) */
.actions{ padding:10px 26px 26px; }
.print-btn{
  background:#224e7a; color:#fff; border:none; border-radius:6px; padding:10px 18px; font-weight:700; cursor:pointer;
}
...
<div class="page">
  <div class="paper">
      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>Total of all earlier payments</td>
            <td class="t-right"><?= peso($prev_paid) ?></td>
          </tr>
          <tr>
    ...
    <div class="footer">
      <div class="sig">
        <div class="line"></div>
        <div class="who">Authorized Representative</div>
      </div>
    </div>
