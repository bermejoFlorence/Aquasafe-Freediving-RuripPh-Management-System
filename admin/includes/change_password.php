<?php
session_start();
require_once '../../db_connect.php'; // adjust kung iba ang path
...
// Client-side validations are good, pero server-side pa rin ang final say:
if ($new !== $confirm) {
    flash_and_redirect('error', 'New passwords do not match.');
}
if (strlen($new) < 6) {
    flash_and_redirect('error', 'Password must be at least 6 characters.');
}
