...
try{
  ...
  $stmt = $conn->prepare("SELECT total_qty, damaged_qty FROM inventory_item WHERE item_id=? FOR UPDATE");
  $stmt->bind_param('i',$item_id);
  $stmt->execute();
  $stmt->bind_result($total,$damaged);
  if(!$stmt->fetch()){ $stmt->close(); $conn->rollback(); fail('Item not found',404); }
  $stmt->close();

  if ($damaged <= 0) { $conn->rollback(); fail('No damaged items to write off',409); }
  $qty = min($qty_req, $damaged);
  if ($total < $qty){ $conn->rollback(); fail('Not enough total to write off',409); } // sanity guard

  $stmtU = $conn->prepare("
    UPDATE inventory_item
       SET damaged_qty = GREATEST(damaged_qty - ?, 0),
           total_qty   = GREATEST(total_qty - ?, 0),
           updated_at  = NOW()
     WHERE item_id = ?
  ");
